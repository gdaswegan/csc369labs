...
def convertFromDT(val):
   return val.strftime("%d-%b-%Y")

# Almeta
rJoinC = customersFrame.join(recieptsFrame, on="CId")
almeta = rJoinC.filter("LastName = 'DOMKOWSKI'").filter("FirstName = 'ALMETA'")

# Gongolais
gJoinI = goodsFrame.join(itemsFrame, on="GId")
rJoinI = recieptsFrame.join(itemsFrame, on="Reciept")
fullJoin = rJoinI.join(gJoinI, on="Reciept")
gongolais = fullJoin.filter("Food = 'Cookie'").filter("Flavor = 'Gongolais'")

almetaRdd = almeta.rdd.map(list)
gongolaisRdd = gongolais.rdd.map(list)

almetaDates = almetaRdd.map(alDates)
gongolaisDates = gongolaisRdd.map(goDates)

dateList = almetaDates.union(gongolaisDates).distinct()
