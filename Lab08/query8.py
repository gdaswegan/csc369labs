...
joined = recieptsFrame.join(itemsFrame, on="Reciept")\
          .join(goodsFrame, on="GId")\
          .join(customersFrame, on="CId")

grouped = joined.groupBy("LastName", "FirstName")
summed = grouped.sum("Price").withColumnRenamed("sum(Price)", "TotalSpent")
